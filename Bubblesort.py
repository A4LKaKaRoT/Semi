...
class Bubblesort:
    def __init__(self, root):
        ...
        self.canvas.create_text(350, 200, text="BUBBLESORT", font=("Helvetica", 24), fill="blue")
    ...
    def update_timer(self):
        while self.timer_running:
            ...
            self.label_timer.config(text=f"Zeit: {elapsed_time:.3f}s")
            time.sleep(0.0001)
    ...
    def bubblesort(self):
        print('start sort')
        mix = True
        t = self.scale_update.get() * self.b_tempo
        n = len(self.bars)
        while mix:
            print('while')
            mix = False
            for j in range(n - 1):
                if self.bars[j] > self.bars[j + 1]:
                    ...
                    self.bars[j + 1] = a
                    mix = True
                    # Hervorhebung, falls ein Tausch stattgefunden hat
                    self.highlight_bars(j, j+1)
                    self.update_canvas()
                    self.canvas.update()
                    self.canvas.after(t)
            # Zeichne die aktuelle Bars nach jedem Durchlauf
            
            self.update_canvas()
            self.canvas.update()
            self.canvas.after(t)
        # Stelle sicher, dass das endgültige Ergebnis angezeigt wird
        self.update_canvas()
        self.canvas.update()
        self.canvas.after(t)
